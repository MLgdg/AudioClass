...
trainloader = torch.utils.data.DataLoader(traindata, batch_size=256, shuffle=True, num_workers=10, collate_fn=data.collate_fn)
testdata = data.AudioData(test)
testloader = torch.utils.data.DataLoader(testdata, batch_size=128, shuffle=False, num_workers=10, collate_fn=data.collate_fn)
device = "cuda:0"
...
model = torch.nn.DataParallel(audiomodel.AudioModel(audio_pre, audio_trunks, audio_head, audio_post).to(device), [0,1,2,3])
...
def validate(val_loader, model, criterion1, epoch=0):
    ...
    for b, nids, audio_data, labels in enumerate(val_loader):
        ...
        acc = accuracy(out, labels.to(device))
        losses.update(loss.data, len(nids))
        top1.update(acc[0], len(nids))
